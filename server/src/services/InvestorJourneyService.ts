...
export type InvestorStage = 'BEGINNER' | 'LEARNER' | 'SIMULATOR' | 'ACTIVE' | 'ADVANCED';
...
const STAGE_THRESHOLDS: [InvestorStage, number][] = [
    ['BEGINNER', 0],
    ['LEARNER', 100],
    ['SIMULATOR', 500],
    ['ACTIVE', 2000],
...
const STAGE_META: Record<InvestorStage, { color: string; benefits: string[]; next: string }> = {
...
export class InvestorJourneyService {
    ...
    static async awardXp(userId: string, action: string) {
    ...
    private static computeStage(xp: number): InvestorStage {
        let stage: InvestorStage = 'BEGINNER';
