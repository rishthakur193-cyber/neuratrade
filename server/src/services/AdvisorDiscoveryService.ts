...
export class AdvisorDiscoveryService {
    static async discoverAdvisors(userId: string) {
        ...
        const mapped = advisors.map(adv => {
            const strategy = (adv.strategyDNA?.strategyType as StrategyType) ?? 'SWING';
    ...
    private static buildReasons(trust: number, comp: number): string[] {
        const r: string[] = [];
        if (trust >= 85) r.push('Highly trusted by the platform');
        if (comp >= 75) r.push('Excellent match for your risk profile');
