...
export class RecoveryPathService {
    static async getProgress(userId: string) {
        ...
        const stage = progress.currentStage as RecoveryStage;
    ...
    static async advanceStage(userId: string) {
        const p = await this.getProgress(userId);
        if (!p.isReadyToAdvance) throw new Error('Requirements not yet met');

        const next: Record<string, string> = { LEARN: 'SIMULATE', SIMULATE: 'INVEST', INVEST: 'INVEST' };
    ...
    private static getNextRequirement(stage: RecoveryStage, winRate: number, total: number): string {
        if (stage === 'LEARN') return 'Score ≥ 70 on assessment';
        if (stage === 'SIMULATE') return `Win rate ≥ 55% over 10+ trades (current: ${winRate}% over ${total})`;
        return 'Full capital access unlocked ✅';
    }

    private static checkReadiness(stage: RecoveryStage, learnScore: number, winRate: number, total: number, discipline: number): boolean {
        if (stage === 'LEARN') return learnScore >= 70;
        if (stage === 'SIMULATE') return total >= 10 && winRate >= 55 && discipline >= 65;
        return false;
    }
}
