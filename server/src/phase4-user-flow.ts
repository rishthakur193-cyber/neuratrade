...
async function testPhase4() {
    ...
    try {
        ...
        const pending = searchRes.data.verificationQueue.find((a: any) => a.name === `Advisor ${testId}`);
        if (!pending) throw new Error('Advisor not found in verification queue');
        ...
        const advisorMatch = marketplace.data.find((m: any) => m.name === `Advisor ${testId}`);
        if (advisorMatch) {
            console.log('✅ Found Advisor in Marketplace. Match Score:', advisorMatch.matchScore, '%');
            console.log('--- PHASE 4 COMPLETED SUCCESSFULLY ---');
        } else {
            console.error('❌ Advisor not found in matching discovery. Current list:', marketplace.data.map((m: any) => m.name));
            process.exit(1);
        }
