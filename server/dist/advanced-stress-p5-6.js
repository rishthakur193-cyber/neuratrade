...
async function runPhase5And6() {
    ...
    for (let i = 0; i < activeSignals.length; i++) {
        ...
        closurePromises.push(SignalService.closeSignal(signal.id, simulatedExitPrice));
    }
    try {
    ...
    for (const advisorId of uniqueAdvisorIds) {
        ...
        if (perf && trust) {
            ...
            if (perf.totalTrades === 0)
                console.error('❌ Performance engine didn\'t tally trades.');
            if (trust.overallScore === 0)
                console.error('❌ Trust score engine didn\'t compute score.');
        }
        else {
