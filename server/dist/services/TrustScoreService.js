...
export class TrustScoreService {
    static async computeAndSave(advisorId) {
        ...
        const hasStopLoss = trades.filter((t) => t.stopLoss && t.stopLoss > 0 && t.stopLoss < t.entryPrice);
        const slRatio = trades.length > 0 ? hasStopLoss.length / trades.length : 0.9;
        const riskManagement = Math.round((slRatio * 6 + ((10 - Math.min(10, dna?.historicalMaxDrawdown ?? 10)) / 10) * 4) * 10) / 10;
    ...
    static async getTrustScore(advisorId) {
